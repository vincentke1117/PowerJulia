...
const WEB_DIST = joinpath(ROOT, "web", "dist")
...
@post "/api/pf" function (req::HTTP.Request)
    response = JGDO.run_pf(String(req.body))
    snapshot_if_ok(response)
    return json_response(response)
end

@post "/api/reconfig" function (req::HTTP.Request)
...
@get "/api/examples/{name}" function (req::HTTP.Request, name::String)
